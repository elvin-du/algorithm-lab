...
func FindAllPath(bt *BinaryTree, target int) [][]int {
	allPath := [][]int{}
	path := []int{}
...
func findAllPath(node *BinaryTreeNode, target, curSum int, allPath *[][]int, path []int) {
	path = append(path, node.Value.(int))
	curSum += node.Value.(int)
	isLeaf := IsNil(node.Left) && IsNil(node.Right)
